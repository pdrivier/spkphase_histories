...
from crossval_fns import train_test_split_bytrials, train_test_split_prespecified
...
def kdePhase_logregHistory_models(df,cell_id,test_size,kde_cvsplits,kde_gridsize,kde_narrow,kde_wide,kde_nbw,
    ...
    """logodds_completemdls: boolean,
                            0: refit the history time lag coefficients for complete models,
                            1: fit coeffs to logodds(history probs)"""


    n_trials = len(set(df['trial_labels'].to_list()))

    train, test = train_test_split_bytrials(df,test_size,kde_cvsplits)
    col_name = 'trial_labels'
    # train, test = train_test_split_prespecified(df,col_name,current_train_labels) #switch test_size to current_train_labels as input to function

    if train['spikes'].sum() >= kde_cvsplits:
